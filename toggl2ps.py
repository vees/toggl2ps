...
def convert_json(weekending, apikey):
	try:
		...
		return json.dumps({
			'jobs' : [],
			'entries' : [],
			'errors' : ['Invalid inputs or parse failed']},
			sort_keys=True,
			indent=4)
...
def convert_json_internal(weekending,apikey):
	...
	end_date = (
		weekending+datetime.timedelta(days=1)-datetime.timedelta(seconds=1)
		).isoformat() + "-04:00"
	start_date = (
		weekending-datetime.timedelta(days=6)
		).isoformat() + "-04:00"
	...
	r = urllib2.Request(
		"https://www.toggl.com/api/v6/time_entries.json?%s" % params)
	base64string = base64.encodestring(
		'%s:%s' % (apikey, 'api_token')).replace('\n', '')
	...
	for entry in weektime["data"]:
		...
		try:
			(dayofweek,project,minutes) = (
				str(startdate.date()), 
				"00000"+str(entry["project"]["name"][0:5]), 
				entry["duration"])
		except:
			errorlist.append(
				"Warning: no project associated with {0} ({1})".format(
		...
		entrylist[dayofweek][project] = (
			entrylist[dayofweek][project] + minutes)

	for x in entrylist.keys():
		for y in entrylist[x].keys():
			entrylist[x][y] = str(ceil(
				(entrylist[x][y]/3600.0)*4)/4) if (y in entrylist[x]) else '0'

	if len(entrylist.keys())>0:
		jobslist = sorted(list(set(reduce(
			lambda x,y:x+y,[entrylist[x].keys() 
			for x in sorted(entrylist.keys())]))))
	else:
		jobslist = {}
